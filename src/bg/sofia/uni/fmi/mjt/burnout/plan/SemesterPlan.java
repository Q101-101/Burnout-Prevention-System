...
public record SemesterPlan(UniversitySubject[] subjects, SubjectRequirement[] subjectRequirements, int minimalAmountOfCredits) {
    ...
    public SemesterPlan {
        if (subjects == null) {
            throw new IllegalArgumentException("Semester plan's subjects is null");
        }
        if (subjectRequirements == null) {
            throw new IllegalArgumentException("Semester plan's subjects requirements is null");
        }
        if (minimalAmountOfCredits < 0) {
            throw new IllegalArgumentException("Semester plan's minimal amount of credits is negative");
        }
    }
