...
public record SubjectRequirement(Category category, int minAmountEnrolled) {
    ...
    public SubjectRequirement {
        if (category == null) {
            throw new IllegalArgumentException("Subject requirement's category is null");
        }
        if (minAmountEnrolled < 0) {
            throw new IllegalArgumentException("Subject requirement's minimum amount enrolled is negative");
        }
    }
